...
public class WorkerThread_New implements Runnable {

    private static final Logger logger = LogManager.getLogger();
    private static int bufferLength = 10;
    //    PConnection conn;
    ...
    List<DataChunk> workerBuffer = new ArrayList<>(bufferLength);
    ...
    private byte[] data_block = new byte[Constants.BLOCK_SIZE_Bytes * 1024 * 1024]; // 32MB for now.
    ...
    public WorkerThread_New(String workerID, String RAID, int pathID, LinkedBlockingQueue<CTRL_to_WorkerMsg> inputQueue,
...
//        logger.info("WorkerThread {} created, src port {}", this.connID, this.dataSocket.getLocalPort());
    ...
    public void run() {
        ...
        rateLimiter.setRate(Constants.DEFAULT_TOKEN_RATE / bufferLength);
...
//                            sendHeartBeat();
    ...
    private void sendData_BestEffort() {

        if (subscribers.size() > 1) {
            logger.error("Not supporting multi-files for now!");
            return;
        }
    ...
    private void processMessage(CTRL_to_WorkerMsg m) {
        ...
        while (m != null) {
            ...
            if (m.getType() == CTRL_to_WorkerMsg.MsgType.SYNC) {
                ...
                // don't simply put a rate here, put chunks into the queue

//                prepareChunks(subscribers, workerBuffer);
...
//                }
    ...
    private void prepareChunks(HashMap<String, SubscriptionInfo> subscribers, List<DataChunk> workerBuffer) {
        double totalRate = sharedData.subscriptionRateMaps.get(raID).get(pathID).values()
                .stream().mapToDouble(SubscriptionInfo::getRate).sum();

        for (Map.Entry<String, SubscriptionInfo> se : subscribers.entrySet()) {
            int number = (int) (se.getValue().getRate() / totalRate);

            for (int i = 0; i < number; i++) {
                DataChunk data = se.getValue().fgi.getDataQueue().remove();

                if ( data != null){
                    workerBuffer.add(data);
                }
